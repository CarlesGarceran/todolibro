...
SET (ISBN = :newISBN, Name = :newName, Publisher = :newPublisher, Author = :newAuthor, Image = :newImage, LaunchDate = :newLaunchDate, Price = :newPrice, Synopsis = :newSynopsis)
WHERE ISBN = :inISBN
");

try {

    function checkIsLibro(mixed $any): bool
    {
        if (!isset($any['ISBN']))
            return false;
    ...
    if (checkIsLibro($userData) == false)
    ...
    $sqlStatement->bindValue(":newLaunchDate", $userData['LaunchDate']);
