...
define("GET_PUBLISHER_FROM_BOOK", "SELECT Publisher FROM Books WHERE ISBN = :inISBN;");
define("GET_BOOKS_BY_PUBLISHER", "SELECT * FROM Books WHERE Publisher = :publisherId GROUP BY ISBN;");
...
try {
    ...
    $recommmandedBooks = invoke(function ($salesBooks, $userId) {
        ...
        foreach ($salesBooks as $sale) {
            $statement = $sqlHandler->prepare(GET_PUBLISHER_FROM_BOOK);
            $statement->bindValue(":inISBN", $sale['ISBN']);

            $statement->execute();

            array_push($publishers, $statement->fetch());
        }

        foreach ($publishers as $publisher) {
            $statement = $sqlHandler->prepare(GET_BOOKS_BY_PUBLISHER);
            $statement->bindValue(":publisherId", $publisher['Publisher']);
